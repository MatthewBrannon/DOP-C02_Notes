...
- - Step Functions are based on the concepts of **tasks** and **state machines**.
    ...
    - A state is referred to by its _name_, which can be any string, but which must be unique within the scope of the entire state machine. An instance of a state exists until the end of its execution.
        - There are 8 types of states:
            - Task state – represents a single unit of work done by your state machine. It could be making API calls to AWS services, such as running an [Amazon Athena](https://tutorialsdojo.com/amazon-athena/) query, writing to a [DynamoDB](https://tutorialsdojo.com/amazon-dynamodb/) table, or invoking a Lambda function to perform some custom processes.
...
![AWS Step Functions Diagram 2](https://td-mainsite-cdn.tutorialsdojo.com/wp-content/uploads/2019/05/AWS-Step-Functions-Diagram-2.jpg)
...
- Security and Monitoring
    ...
    - Step Functions delivers real-time diagnostics and dashboards, integrates with [Amazon CloudWatch](https://tutorialsdojo.com/amazon-cloudwatch/) and [AWS CloudTrail](https://tutorialsdojo.com/aws-cloudtrail/), and logs every execution, including overall state, failed steps, inputs, and outputs.
...
- - Step Functions can help ensure that long-running, multiple ETL jobs execute in order and complete successfully, instead of manually orchestrating those jobs or maintaining a separate application.
    ...
    - You can also orchestrate data and services that run on [Amazon EC2](https://tutorialsdojo.com/amazon-elastic-compute-cloud-amazon-ec2/) instances, containers, or on-premises servers.